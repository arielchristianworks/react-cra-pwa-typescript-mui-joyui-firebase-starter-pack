    ...
    <Fragment>
      ...
      >
        <Typography
          ...
          level='body-md'
          component='span'
          fontWeight={!Boolean(props.depth) ? 'bold' : 'light'}
        ...
        >
          <Typography component='span' color={!Boolean(props.depth) ? 'primary' : 'neutral'}>
            {'# '}
          </Typography>
          {props.item.label}
        </Typography>
      ...
      {Boolean(props.item.children) ? (
        <Fragment>
          {props.item.children?.map((chd, index2) => (
            <StickySideNavItem
              key={`ci-${props.index}.${index2}`}
              depth={props.depth + 1}
              index={`${props.index}.${index2}`}
              item={chd}
